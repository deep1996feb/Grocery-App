...
class ProductView(View):
	def get(self, request):
		...
		return render(request, 'home.html', {'Kitchen':Kitchen, 'Vegetables':Vegetables, 'Fruits':Fruits, 'totalitem':totalitem})
...
def fruits(request, data=None):
	...
	if data==None :
			Fruits = Product.objects.filter(category='F')
	elif data == 'Mango' or data == 'Mangoes':
			Fruits = Product.objects.filter(category='F').filter(brand=data)
	elif data == 'below':
			Fruits = Product.objects.filter(category='F').filter(discounted_price__lt=100)
	elif data == 'above':
			Fruits = Product.objects.filter(category='F').filter(discounted_price__gt=100)
